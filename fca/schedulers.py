...
class PlateauTracker:
    ...
    def update(self, loss, acc):
        """
        Returns true when plateau has been reached.
        """
        if self.patience is None or not self.plateau or self.plateau<=0:
            return False
        measure = acc if self.measure=="acc" else loss
        if measure < (self.best_measure-self.plateau):
            self.best_val_loss = loss
            self.best_val_acc = acc
            self.best_measure = acc if self.measure=="acc" else loss
            self.counter = 0
        else:
            self.counter += 1
