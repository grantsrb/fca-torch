...
class TestFunctionalComponentAnalysis(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertFalse(self.fca.is_cached)
    ...
    def test_set_cached(self):
        self.fca.set_cached(cached=True)
        self.assertTrue(self.fca.is_cached)

    def test_reset_cached_weight(self):
        self.fca.add_component()
        self.fca.reset_cached_weight()
        self.assertIsNotNone(self.fca.cached_weight)
    ...
    def test_reset_cached_weight(self):
        ...
        self.fca.reset_cached_weight()
        self.assertIsNotNone(self.fca.cached_weight)
        self.assertAlmostEqual(((self.fca.cached_weight-self.fca.make_fca_matrix())**2).mean().item(), 0, places=5)

    def test_update_parameters_no_grad(self):
        self.fca.add_component()
        self.fca.update_parameters_no_grad()
        self.assertEqual(len(self.fca.parameters_list), 2)

    def test_make_matrix(self):
        self.fca.add_component()
        matrix = self.fca.make_fca_matrix()
